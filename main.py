...
async def send_to_private_bot(phrase: str, user_id: int):
    try:
        ...
        logger.error(f"Failed to send to private bot: {str(e)}")
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.error(f"Update {update} caused error {str(context.error)}")
...
def run_bot():
    try:
        ...
        logger.error(f"Telegram bot failed with exception: {str(e)}\n{traceback.format_exc()}")
